...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [10, 30, 5, 1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        """
        # TODO DONE (change player's gravity here by calling self.change_gravity)
        nn_input = []

        nn_input.append(player_x / screen_width)
        nn_input.append(player_y / screen_height)

        for k in range(4):
            if len(obstacles) > k:
                nn_input.append(obstacles[k]['x'] / screen_width)
                nn_input.append(obstacles[k]['y'] / screen_height)
            else:
                nn_input.append(1.)
                nn_input.append(0.)
